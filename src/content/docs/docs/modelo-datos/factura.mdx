---
title: Modelo de datos — Generador
description: El contrato de datos que usa generateXmlInvoice(...) para generar comprobantes tipo factura válidos para el SRI.
---

import { Aside } from '@astrojs/starlight/components';

Este es el contrato de datos usado por la función `generateXmlInvoice(...)` para generar
comprobantes tipo **Factura** válidos para el SRI.
...
class ComprobanteModel {
  ...
  /** Anexo 26 — RUC del proveedor del sistema de facturación (13 dígitos) */
  rucProveedor?: string;
}
...
<Aside type="note" title="Sobre rucProveedor">
No genera un nodo propio en el XML: la librería lo emite como
`<campoAdicional nombre="RUC Proveedor">` dentro de `infoAdicional`, según el
**Anexo 26**. [Ver ejemplo completo](../../generar/#anexo-26--ruc-del-proveedor).
</Aside>
...
## Respuesta de `generateXmlInvoice(...)`

Esta función devuelve una promesa con el XML generado como cadena, y además el objeto
convertido a JSON con la estructura del comprobante.

```ts title="ResponseGenerateXmlModel.ts"
